...
from schemas.order import OrderSchema, OrderStatusSchema
...
async def place_an_order(order: OrderSchema, Authorize: AuthJWT = Depends()):
    """
    ## Placing an Order
    ...
    user = session.query(UserModel).filter(UserModel.username == current_user).first()

    new_order = OrderModel(quantity=order.quantity)
...
async def get_order_by_id(id: int, Authorize: AuthJWT = Depends()):
    ...
    current_user = session.query(UserModel).filter(UserModel.username == user).first()

    if current_user.is_staff:
        order = session.query(OrderModel).filter(OrderModel.id == id).first()
...
async def get_specific_order(id: int, Authorize: AuthJWT = Depends()):
    ...
    current_user = (
        session.query(UserModel).filter(UserModel.username == subject).first()
    )
...
async def update_order(id: int, order: OrderSchema, Authorize: AuthJWT = Depends()):
    """
    ## Updating an order
...
async def update_order_status(
    id: int, order: OrderStatusSchema, Authorize: AuthJWT = Depends()
):
